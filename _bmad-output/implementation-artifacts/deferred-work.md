## Deferred from: PR #104 CI honesty restoration (2026-05-19)

This is the residual layer of the CI-honesty onion peeled in PR #104. Five layers above were fixed across PR #102 (CI pipefail + HyzerKit Sendable) and PR #104 (Xcode 16.2 → 26.2 bump, RoundSignatureRenderingTests fixture, HyzerAppTests double-linking). This bullet captures what remains:

- **HyzerApp test host signal-traps at bootstrap on iOS 26.2 simulator (CI), unknown root cause.** After all five upstream fixes, `xcodebuild test` builds cleanly under Xcode 26.2, deploys HyzerApp.app + HyzerAppTests.xctest to an iOS 26.2 simulator, but the test runner reports `HyzerApp encountered an error (Early unexpected exit, operation never finished bootstrapping - no restart will be attempted. Underlying Error: Test crashed with signal trap before establishing connection.)` before any test method runs. The HyzerKit duplicate-class warnings that caused the previous bootstrap trap are GONE (verified by grep in CI logs after the project.yml linkage fix), so this is a different crash. Candidates (not yet diagnosed): (a) SwiftData/CloudKit container init failing in test env — `CoreData: error: Failed to stat path '.../OperationalStore.store'` appears repeatedly in the log but may be benign; (b) missing entitlement or Info.plist key under iOS 26.2 SDK; (c) Swift runtime issue surfaced by Xcode 26.2 toolchain (newer Swift than locally validated); (d) launch-time crash in `HyzerApp.init` / `AppServices` setup. Investigation requires either downloading the CI xcresult artifact and inspecting the crash log, OR running `xcodebuild test` locally on a Mac with a compatible simulator runtime. Reference CI runs: `26111275168` (job `76789129832`), `26113278933` (job `76796029805`). Local-build constraint surfaced earlier in the session ("HyzerApp can't be built on this version of macOS") may be related but independent.

## Deferred from: code review of story-15.2 (2026-05-19)

- CLAUDE.md "Project Status" — `HyzerWatch has no test target` wording is fact-true but underspecified. AC #6's format reserves `N3 HyzerWatch`; a parenthetical confirming the project.yml audit (e.g., `(no HyzerWatch test target exists in project.yml — confirmed 2026-05-18)`) would improve fidelity. Pre-existing wording; defer to a future CLAUDE.md polish pass.
- CLAUDE.md "Project Status" line dropped the `23/23 stories` count without replacement. Old line: `Epics 1–8 complete — 23/23 stories, 407 tests`. New line: `Epics 1–14 complete — Epic 15 (pre-launch hardening) in progress`. Story-count fidelity is lost. Append `(N/N stories across epics 1–14)` once known. Defer.
...
- ~~`project.pbxproj` regeneration not present in diff.~~ _Resolved 2026-05-19 by PR #96 follow-up — `HyzerApp/App/Info.plist` added to `.gitignore` and untracked via `git rm --cached`. The file is now treated as a build artifact, regenerated by `xcodegen generate` (CI step + local project.yml-watching hook). `project.yml info.properties` is the sole editable surface for the merged Info.plist keys. See Story 15.5 spec "Build-System Decision (Resolved 2026-05-19)" section._
- Light-variant `#FFFFFF` in `HyzerApp/Resources/Assets.xcassets/LaunchBackground.colorset/Contents.json` is a hardcoded literal rather than a `ColorTokens.backgroundPrimaryLight` reference. Acceptable today because `UIUserInterfaceStyle = Dark` is pinned (the light variant is unreachable in production). **Conditional future work:** if light-mode is ever un-pinned, introduce `ColorTokens.backgroundPrimaryLight` in `HyzerKit/Sources/HyzerKit/Design/ColorTokens.swift` and regenerate `LaunchBackground.colorset` from that token to maintain the "design tokens only" coupling.
...
- ~~Run canonical `xcodebuild test -project HyzerApp.xcodeproj -scheme HyzerApp -destination 'platform=iOS Simulator,name=iPhone 17 with Watch'` against the 407-test baseline (AC7).~~ _Resolved 2026-05-19 by Story 15.2 — canonical baseline reconciled in CLAUDE.md "Project Status." CI now runs `xcodebuild test` on every PR via `.github/workflows/test.yml`, so the manual deferral is obsolete. (Story 15.2 PR #94.)_
- ~~Pin `UIUserInterfaceStyle: Dark` in `project.yml info.properties` + `HyzerApp/App/Info.plist`.~~ _Resolved 2026-05-19 by Story 15.5 — dark mode pinned in project.yml; Info.plist now a build artifact. (Story 15.5 PR #96.)_
