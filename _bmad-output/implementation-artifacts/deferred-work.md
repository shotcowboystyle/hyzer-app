...
- ScoreEvent fetch `fetchLimit = maxRounds * 20` multiplier under-bounds for multi-course users. ScoreEvent fetch is NOT filtered by `courseID`, so a player with 500 rounds spread across multiple courses (10 courses × 60 rounds × 18 holes = 10,800 events) hits the 10,000 cap and silently drops the oldest events. Practical risk is low at PMVP scale. (`HyzerKit/Sources/HyzerKit/Domain/PersonalBestService.swift:68`)
...
- Test helper `insertRound(... completedAt: Date(timeIntervalSinceNow: -1) ...)` produces near-identical timestamps for sibling inserts in the same test. SortDescriptor on identical `completedAt` can shuffle insertion order non-deterministically. Pre-existing test pattern; audit alongside the `ValueCollector` shared-test-helper extraction debt called out in CLAUDE.md "Known Technical Debt". (`HyzerKit/Tests/HyzerKitTests/Domain/PersonalBestServiceTests.swift:23`)
