...
## Deferred from: code review of story-15.5 (2026-05-19)

- `project.pbxproj` regeneration not present in diff. After applying the AC3 Info.plist strip, `xcodegen generate` was re-run cleanly; pbxproj had no delta because UI* key reordering in `project.yml` does not affect the pbxproj contents (Info.plist is referenced by path). XcodeGen's "Generating plists" step now rewrites `HyzerApp/App/Info.plist` from the merged `project.yml info.properties` source-of-truth on every generate, so the stripped Info.plist will be repopulated as a generated artifact at build time. The on-disk Info.plist therefore visibly contains the keys post-generate, but `project.yml` is now the unambiguous editable surface.

## Deferred from: code review of 14-2-generative-visual-round-signature-on-summary-card (2026-05-18)

- Manual verification (Tasks 8.1–8.5) was not performed by the dev agent per Completion Notes. Includes the palette-on-`backgroundElevated` contrast spot-check (`Color.textSecondary`, `Color.backgroundTertiary` are at higher risk of failing 4.5:1 AA against the elevated background). Spec line 425 explicitly required this check before proceeding past Task 4. Recommend human verification on simulator (live + AirDrop PNG + VoiceOver + Reduce Motion) before merge. **Note (2026-05-19, Story 15.3 review patch):** Story 15.3's AC #2 automated contrast spot-check **confirms** this risk rather than resolves it — `backgroundTertiary` at 1.21:1 is a real AC #3 failure affecting approximately 37.5% of generated signatures because `RoundSignature.swift:114-117` includes `backgroundTertiary` in the foreground palette. This bullet remains open pending (a) human simulator verification of the remaining manual ACs (tracked by PR #95 / Story 15.3 Pending Handoff section) and (b) a separate follow-up story to remediate `backgroundTertiary` in the palette array. **Concrete remediation options (for the follow-up story to choose among):**
  1. **Drop to 7-color palette** — remove `backgroundTertiary` from `RoundSignature.swift:116`; smallest diff; reduces visual variety of generated signatures.
  2. **Replace with `destructive` (#FF3B30)** — passes WCAG against `backgroundElevated`; visually distinct from but close to `scoreWayOver` (#FF453A) which could weaken the score-band visual encoding. Acceptable if Story 14.2's design intent treats the signature palette as decorative rather than carrying score-band semantics.
  3. **Outline-only treatment for `backgroundTertiary`** — keep the token in the 8-color palette but treat it as a stroke-only colour (no fill, no gradient stop) since outlines have a lower contrast bar than fills. Requires guarding the palette consumer in `concentricRings` / `flourish` against fill paths when this token is selected. Largest code diff.
  
  Recommend option 1 (drop to 7-color) as the minimal-risk default unless Story 14.2 design intent prefers visual variety. Filing the follow-up story should restate the choice and the rationale.
- **Story 15.3** — Story 14.2 manual verification pending: contrast calculations complete (all auto-checkable via code); live-simulator verifications (screenshots, VoiceOver utterance, Reduce Motion comparison) still required from a human developer with simulator access. Status: `blocked-on-human-ops` (introduced 2026-05-19 by PR #100).
